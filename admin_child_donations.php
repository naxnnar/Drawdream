...
$sponsorDisplayList = drawdream_child_foundation_sponsor_display_list($conn, $childId, $childCategoryId);
...
<div class="admin-directory-page children-admin-directory">
    <div class="admin-directory-head">
        <a href="children_.php" class="admin-directory-back">← กลับหน้า Children</a>
        <h1 class="admin-directory-title">ยอดบริจาคเด็ก</h1>
        <p class="admin-directory-desc">
            การจัดการด้านการเงินและความโปร่งใส: ตรวจสอบเส้นทางเงินของเด็กแต่ละคน และดูประวัติการบริจาคที่เด็กได้รับจริง
        </p>
    </div>

    <div class="admin-directory-actions-hint">
        <strong>เด็ก:</strong> <?= htmlspecialchars((string)($child['child_name'] ?? '-')) ?>
        · <strong>มูลนิธิ:</strong> <?= htmlspecialchars((string)($child['display_foundation_name'] ?? '-')) ?>
        <?php if ($sponsorDisplayRows !== []): ?>
            · <strong>ผู้บริจาค / สถานะ:</strong>
            <?php
            $sponsorHintParts = [];
            foreach ($sponsorDisplayRows as $sr) {
                $sponsorHintParts[] = trim((string)($sr['name'] ?? '')) . ' — ' . trim((string)($sr['status_label'] ?? ''));
            }
            echo htmlspecialchars(implode(' · ', $sponsorHintParts));
            ?>
        <?php endif; ?>
    </div>
    ...
    <div class="admin-directory-actions-hint" style="margin-top:10px;">
